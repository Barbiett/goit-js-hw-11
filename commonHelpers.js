...
</li>`).join("");d.insertAdjacentHTML("beforeend",r),new g(".gallery a",{captionsData:"alt",captionDelay:250}).refresh()}const h=document.getElementById("search-form"),c=document.getElementById("search-input"),i=document.querySelector(".gallery"),u=document.querySelector(".loader");h.addEventListener("submit",f);function f(n){n.preventDefault();const r=c.value.trim();if(r==="")return l.warning({message:"Please try again! Write something.",position:"topRight",color:"red"});u.classList.remove("is-hidden"),m(r).then(a=>{if(a.hits.length===0){l.error({message:"Sorry, there are no images matching your search query. Please try again!",color:"red",position:"topRight"}),i.innerHTML="";return}i.innerHTML="",p(a.hits)}).catch(a=>{console.error("Error fetching data:",a),i.innerHTML=""}).finally(()=>{u.classList.add("is-hidden")}),c.value=""}
